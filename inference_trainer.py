...
def inference(model: nn.Module, tokenized_sent, device: str):
    ...
    dataloader = DataLoader(
        tokenized_sent, batch_size=512, shuffle=False, drop_last=False
    )
    model.eval()
...
def main(args):
    ...
    tokenizer = load_tokenizer(
        model_type=args.model_type, preprocess_type=args.preprocess_type
    )
...
if __name__ == "__main__":
    MODEL_DIR = "./saved_models/[MODELNAME]_[PRETRAINEDTYPE]_[SAVEID]/checkpoint-XXXX/"
    SAVE_NAME = (
        "./predictions/[PREDICTIONSAVENAME].csv"
    )
